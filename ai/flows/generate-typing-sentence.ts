...
const generateTypingSentenceFlow = ai.defineFlow(
  ...
  async input => {
    console.log('GENERATE TYPING SENTENCE FLOW INPUT:', JSON.stringify(input, null, 2));
    const result = await generateTypingSentencePrompt(input);
    console.log('GENERATE TYPING SENTENCE PROMPT RESULT:', JSON.stringify(result, null, 2));
